...
class BillController extends Controller {
  async add() {
    ...
    try {
      ...
      if (!decode) return;
      user_id = decode.id;
      // user_id 默认添加每个账单项，作为后续获取指定用户的账单的标识
      // 可以理解为，我登录A的账号，那么所做的操作都得加上A的账号标识，后续获取A的信息时，方便过滤
  ...
  async list() {
    const { ctx, app } = this;
    // 获取日期date，分页数据，类型type_id,这些都是我们再前端传给后端的数据
    ...
    try {
      ...
      const listMap = _list.reduce((curr, item) => {
        ...
        // 如果能在累加的数组中找到当前项日期date，那么在数组中的加入当前项到bills数组
        if (curr && curr.length && curr.findIndex(item => item.date == date) == -1) {
          const index = curr.findIndex(item => item.date == date);
          curr[index].bills.push(item);
        }
        // 如果在累加的数组中找不到当前项目日期的，那么再新建一个
        if (curr && curr.length && curr.findIndex(item => item.date == date) == -1) {
          curr.push({
            date,
            bills: [item],
        ...
        return curr;
      }, []).sort((a, b) => moment(b.date) - moment(a.date)); // 排序，时间倒叙

      // 分页处理，listMap 为我们处理完成的全部数据，还没有分页
      const filterListMap = listMap.slice((page - 1) * page_size, page * page_size);

      // 计算当月总收入和支出
      // 首先获取当月的所有账单列表
      const total_list = list.filter(item => moment(Number(item.date)).format('YYYY-MM') == date);
      // 累加计算支出
      const totalExpense = total_list.reduce((curr, item) => {
        if (item.pay_type == 1) {
      ...
      // 累加计算收入
      const totalIncome = total_list.reduce((curr, item) => {
        if (item.pay_type == 2) {
          curr += Number(item.amount);
          return curr;
        }
  ...
  async delete() {
    ...
    try {
      ...
      if (!decode) return;
      user_id = decode.id;
      const res = await ctx.service.bill.delete(id, user_id);
      ctx.body = {
        code: 200,
        msg: '请求成功',
        data: null,
